...
// ─── beautify-select 手帐风下拉组件（plugin-kit 积木，零依赖） ───
...
(function () {
  ...
  function render() {
    ...
    html += '<button class="modal-btn cancel" id="modal-cancel" onclick="window._tbClose()">取消</button>';
    ...
    // 手帐风下拉：所有原生 select 统一走 beautify-select（含装饰商店、模型设置）
    if (window.beautifySelect) {
      var selList = app.querySelectorAll('select');
      for (var si = 0; si < selList.length; si++) beautifySelect(selList[si]);
    }

    // 主弹窗（含装饰商店）：点击空白处关闭，与状态弹窗同一逻辑。
    // 装饰商店模式带 deco-open 标记，空白点击同样走 _tbClose（它负责恢复按钮态）。
    var mainOverlay = document.getElementById('modal-overlay');
    if (mainOverlay && !mainOverlay.dataset.ddBlankClose) {
      mainOverlay.dataset.ddBlankClose = '1';
      mainOverlay.addEventListener('click', function(e) {
        if (e.target === mainOverlay) window._tbClose();
      });
    }
  ...
  window._tbClose = function() {
    ...
    if (overlay) {
      ...
      overlay.classList.remove('deco-open');
    }
    var confirm = $('#modal-confirm');
    if (confirm) {
