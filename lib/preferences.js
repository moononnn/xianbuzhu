...
const TAG_DEFAULTS = Object.freeze({
  大方: Object.freeze({ gifts: ["coffee", "bouquet"], scenes: ["desk-tidy"] }),
  敏感: Object.freeze({ gifts: ["flower", "moon"], scenes: ["sticky-note", "desk-lamp"] }),
  冷淡: Object.freeze({ gifts: ["tea", "moon"], scenes: ["desk-lamp", "sticky-note"] }),
  温柔: Object.freeze({ gifts: ["tea", "flower"], scenes: ["jasmine-water", "desk-tidy"] }),
  热情: Object.freeze({ gifts: ["coffee", "cookies", "bouquet"], scenes: ["desk-tidy", "jasmine-water"] }),
  "边界感强": Object.freeze({ gifts: ["tea", "star"], scenes: ["desk-lamp", "sticky-note"] }),
});
...
export function deriveHeartPreferences({ description = "", temperamentTag = "" } = {}) {
  ...
  if (/便签|画画|手作|纸|写字|涂鸦/.test(text)) appendUnique(scenes, ["sticky-note"], 3);
  if (/整理|收拾|桌角|整洁|归置/.test(text)) appendUnique(scenes, ["desk-tidy"], 3);
  if (/浇水|照料|细心|植物/.test(text)) appendUnique(scenes, ["jasmine-water"], 3);
  if (/安静|夜晚|灯光|独处/.test(text)) appendUnique(scenes, ["desk-lamp"], 3);
