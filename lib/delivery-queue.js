...
// 文件预算豁免：投递队列状态机聚合层（入队/落盘/批量合并/超时对账/恢复），
// 拆分会切断同一条队列状态流转的完整时序，与 heartbeat.js 同类保持单体。
...
function comparableText(value) {
  return messageText(value).replace(/\s+/g, " ").trim();
}
...
// session:history 为兼容契约只返回 role/content/entryId，不带时间戳；
// 对账时补读原始 JSONL，只有确认是本批之后的同一条文本才算已送达。
function readRawUserMessages(sessionPath) {
  try {
    ...
    return raw
      ...
      .map((line) => {
        try {
          ...
          return {
            text: comparableText(message.content),
            timestamp: normalizeTimestamp(message.timestamp ?? record.timestamp ?? record.ts),
          };
        } catch {
          return null;
        }
...
export function buildDeliveryBatchText(entries) {
  ...
  return `📬 收到来自${userName}的 ${list.length} 份心意，趁你忙着时先收着：\n${lines.join("\n")}`;
}
...
export class DeliveryQueueManager {
  ...
  async _historyContainsAfter(sessionPath, text, notBefore) {
    ...
    const historyHasText = (history?.messages || []).some(
      (message) => message?.role === "user" && comparableText(message.content) === expected,
    );
    if (!historyHasText) return false;

    const threshold = Number(notBefore);
    if (!(threshold > 0)) return null;
    const rawMessages = readRawUserMessages(sessionPath);
    if (!rawMessages) return null;
    const matching = rawMessages.filter((message) => message.text === expected);
    if (matching.length === 0) return null;
    if (matching.some((message) => message.timestamp != null && message.timestamp >= threshold)) {
      return true;
    }
    if (matching.some((message) => message.timestamp == null)) return null;
    return false;
  }
  ...
  async _reconcileUncertain() {
      ...
      for (const item of data.deliveryQueue || []) {
        ...
        if (!group) {
          ...
          group = {
            batchId,
            sessionPath,
            notBefore: Number(item.sendingAt) || 0,
            entries: [],
          };
          byBatch.set(batchId, group);
        } else if (Number(item.sendingAt) > 0 && (!group.notBefore || Number(item.sendingAt) < group.notBefore)) {
          group.notBefore = Number(item.sendingAt);
        }
        group.entries.push({ ...item });
      }
      if (changed) saveData(data);
    ...
    for (const group of groups) {
      ...
      try {
        const found = await this._historyContainsAfter(
          group.sessionPath,
          text,
          group.notBefore,
        );
        if (found === true) {
          ...
          await this._postponeUncertain(group, "对账缺少可验证的消息时间");
        }
      } catch (error) {
