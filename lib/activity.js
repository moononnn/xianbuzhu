...
export function scanTodayActivity(data) {
  ...
  for (const agentId of partnerIds) {
    ...
    try {
      ...
      const todayMidnight = todayMidnightBJ();
      ...
      sessionFiles = allFiles
        ...
        .sort((a, b) => {
          try {
            return (
              fs.statSync(path.join(sessionsDir, b)).mtime -
    ...
    for (const file of sessionFiles.slice(0, 3)) {
      try {
        ...
        if (!label) {
          ...
          if (sessMatch && titleMap[sessMatch[0]])
            label = titleMap[sessMatch[0]];
        }
        if (label) {
          result[agentId].title =
        ...
        for (const line of lines) {
          try {
            ...
            if (d.type === "message" && d.message?.role === "user") {
              ...
              if (!clean) continue;
              result[agentId].title =
                clean.length > 25 ? clean.slice(0, 25) + "…" : clean;
              break;
            }
          } catch {}
        }
        ...
        for (const line of lines) {
          try {
            ...
            for (const item of contentItems) {
              if (item.type !== "toolCall" || item.name !== "subagent")
                continue;
  ...
  for (const agentId of partnerIds) {
    ...
    try {
      ...
      const allFiles = fs
        .readdirSync(sessionsDir)
        .filter((f) => f.endsWith(".jsonl"));
      // 会话文件名是 UTC 日期，todayStr() 是北京时间：北京 00:00-08:00 之间的
      // 新会话文件名匹配不上，必须用 mtime 兜底（按北京今天 0 点对齐，与 scanTodayActivity 一致）
      const todayMidnight = todayMidnightBJ();
      // 统计今天全部会话（不限制数量：限制会导致活跃用户的工作量被低估，
      // 性能由外层 1 分钟缓存兜底）
              ...
              if (msg.role === "assistant") {
                for (const item of contentItems) {
                  if (item.type === "text" && typeof item.text === "string") {
                    stats.charsOutput += item.text.length;
                  } else if (item.type === "toolCall") {
                    stats.toolCalls++;
                    if (
                      item.name === "file" ||
                      item.name?.startsWith?.("file")
                    ) {
                      stats.fileOps++;
                    }
                    if (item.name === "subagent") {
                      stats.subagentDispatches++;
                    }
                  }
                }
