...
export function scanWorkStats(data) {
  ...
  for (const agentId of partnerIds) {
    const stats = { toolCalls: 0, charsOutput: 0, fileOps: 0, subagentDispatches: 0 };
    const sessionsDir = path.join(AGENTS_DIR, agentId, 'sessions');

    try {
      if (!fs.existsSync(sessionsDir)) { result[agentId] = stats; continue; }

      const allFiles = fs.readdirSync(sessionsDir).filter(f => f.endsWith('.jsonl'));
      const todayFiles = allFiles.filter(f => f.startsWith(ts)).slice(0, 3);
