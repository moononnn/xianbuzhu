...
4. 人情味：像不像这个助手真的会留下的话？有没有真实线索或具体的人在场感？没有近期情境时，一句自然的直接想到也可以通过，不能要求模型为了“在场”新增物件
5. 字数：${lengthRule}
...
`;
  ...
  if (isHeart) {
    ...
    const isContextual = event.id === "contextual-moment";
    const mode = options.expressionMode || {};
    const factSources = Array.isArray(options.interactionSeeds)
      ? options.interactionSeeds.filter((item) => typeof item === "string" && item.trim()).join("；")
      : "";
    const dialect = typeof event.dialect === "string" && event.dialect.trim()
      ? event.dialect.trim()
      : "";
    const dialectRule = dialect
      ? `- 方言保真：当前助手设定明确带有这段方言口癖，可少量自然使用，不要堆叠成表演：${dialect}`
      : "- 方言保真：当前助手设定没有明确方言，不要凭空添加方言口头语。";
    const contextAllowance = factSources
      ? "当前有可引用情境，正文要从其中挑一个落点。"
      : "当前没有可引用情境；正文可以只写一句直接想到对方的话、轻微感受或不依赖物件的观察，不得因为没有物件或物理动作就退稿。";
    prompt += `
这是一份异步心意，专门检查下面几件事：
${isContextual
    ? "- 开放心意：当前没有指定物件，正文要从当天已有线索或一个具体观察落地；日程和天气本身也可以作为落点，直接想到对方或轻轻祝愿即可。不要因为旧样本而自动写便签、浇花、整理桌角或留灯。允许纯粹的观察、想到或轻微玩笑，不要求虚构物件或物理动作。"
    : `- 事件保真：当前事件是“${event.icon || ""}${event.name || "一件小礼物"}”。正文必须能让人认出这件事，不能抽到便签却写成倒水、抽到茶却写成花。`}
- 事实边界：具体物件、饮品、衣物、地点、提醒和已经完成的动作，必须在可引用事实里有明确出处；没有出处就退稿。用户有一个待办或提醒，不代表助手已经替用户设置、挂上或盯住它；待办只能按“要做/准备做/将要发生”来提及。${factSources ? `可引用事实：${factSources}` : "当前没有可引用的近期情境，只能写感受、观察或已有事件，不得补物件。"} ${contextAllowance}
- 空情境优先规则：可引用事实为空时，只要正文没有编造物件或具体事件、没有感谢体和模型腔，即使只有一句有声音的直接想到，也不要以“缺少物件、动作或在场”为理由退稿。
${dialectRule}
...
- 表达方向：${mode.label || "换一个近期没用过的生活入口"}。${mode.instruction || "不要只改几个形容词，要换掉心意的落点。"}
