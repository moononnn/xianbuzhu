...
export function ensureDailyHeartPlan(data, date = todayStr(), random = Math.random) {
  if (data.heartSettings?.enabled === false) return false;
...
// 关闭主动心意时清掉尚未生成/投递的计划，避免重新开启后补发关闭期间错过的旧计划。
// 已经进入信箱的心意不在这里处理，继续保留并按原规则过期。
export function disableHeartPlan(data, now = Date.now()) {
  const plan = data.heartPlan && typeof data.heartPlan === "object" ? data.heartPlan : {};
  ...
  for (const entry of entries) {
    ...
    entry.cancelledAt = new Date(now).toISOString();
    entry.nextAttemptAt = null;
    entry.cancelReason = "disabled";
    changed = true;
  }
  if (plan.date !== null || entries.length > 0) changed = true;
  if (changed) {
    data.heartPlan = {
      date: null,
...
function deliverStagedHearts(data, now, gates) {
  if (data.heartSettings?.enabled === false || !gates?.ok) return false;
...
async function runHeartbeatTick(ctx, options = {}) {
  ...
  let heartEnabled = true;

  await withDataLock(() => {
    const data = loadData();
    if (data.heartSettings?.enabled === false) {
      let changed = disableHeartPlan(data, now);
  ...
  await withDataLock(() => {
    ...
    if (!heart) return;
    if (data.heartSettings?.enabled !== false && gatesSnapshot?.ok) {
