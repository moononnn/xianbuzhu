...
// 文件预算豁免：风铃进程生命周期 + 本地代理 + 目标选择聚合层，拆分会导致代理链路跨文件回环
//
...
// 风铃进程 PID 文件（与 python 侧 PID_PATH 同一路径）：作为「风铃是否真在跑」的事实源。
// 插件重载/重启后 appProcess 句柄会丢，但靠它仍能认出桌面上还活着的球，不再失忆。
const PID_FILE = path.join(HANA_HOME, "data", "work-visit", "fengling.pid");
const TARGET_SESSION_LIMIT = 5;
// 固定目标（pinnedTarget）寿命：24 小时。
// 当初为了让悬浮球把动作稳定发给"某段对话"而设，但没有寿命的话，一旦钉下，
// 哪怕用户早就换去别的窗口，悬浮球也会一直把动作推给那个旧窗口——本次事故正是
// 昨晚钉下的窗口今天还在收互动。到期自动回落到"自动判断最近活跃窗口"；
// 无 pinnedAt 的历史遗留固定目标也一律视为失效，杜绝旧钉子无脑续命。
const PINNED_TARGET_TTL_MS = 24 * 60 * 60 * 1000;
...
function fenglingHeartPayload(heart, data) {
  return {
    ...publicHeart(heart, findResponseVisit(data, heart)),
...
// 固定目标状态：{ pinned, stale }。stale=true 表示钉选已失效（文件不存在/助手不在
// 列表/无 pinnedAt 的历史遗留/超过 24h 寿命），此时调用方应清除它并回落到自动判断。
function pinnedTargetStatus(data) {
  ...
  if (!agentId || !getPartnerIds(data).includes(agentId) || !sessionPath) {
    return { pinned: null, stale: Boolean(pinned?.sessionPath) };
  }
  if (!isSessionPathForAgent(sessionPath, agentId)) {
    return { pinned: null, stale: true };
  }
  const pinnedAt = Number(pinned.pinnedAt || 0);
  if (!(pinnedAt > 0) || Date.now() - pinnedAt > PINNED_TARGET_TTL_MS) {
    return { pinned: null, stale: true };
  }
  return {
    pinned: {
...
// 清除已过期的固定目标：只在有写锁的地方调用。纯读取路径靠 TTL 判空即可。
async function clearStalePinnedTarget() {
  return withDataLock(() => {
    const latest = loadData();
    if (!latest.pinnedTarget) return true;
    const { pinned, stale } = pinnedTargetStatus(latest);
    if (!stale || pinned) return true;
    latest.pinnedTarget = null;
    if (!saveData(latest)) {
      console.warn("[闲不住] 清理过期固定对话失败");
      return false;
    }
    console.log("[闲不住] 固定对话已过期，收回自动判断");
    return true;
  });
}
...
function currentTargetPayload() {
  const data = loadData();
  const { pinned } = pinnedTargetStatus(data);
  if (pinned) return pinned;

  // 风铃动作是用户此刻的点击，不能吃页面轮询缓存，必须重新确认目标会话。
  const id = findMostActiveAgentId(getPartnerIds(data), { force: true });
  if (!id) return null;
...
async function targetPayload(bus) {
  // 先清掉已过期的固定目标，再读当前数据；这样 /target 从不把"已过期"误报成固定。
  await clearStalePinnedTarget();
  const data = loadData();
  const { pinned } = pinnedTargetStatus(data);
  const target = pinned || currentTargetPayload();
  let resolved = target;
  let pinnedExpiresInMs = 0;
  if (pinned) {
    const pinnedAt = Number(data.pinnedTarget?.pinnedAt || 0);
    pinnedExpiresInMs = pinnedAt > 0
      ? Math.max(0, PINNED_TARGET_TTL_MS - (Date.now() - pinnedAt))
      : 0;
  }
...
export async function performFenglingVisit(input, bus, visitFn = performVisit) {
  // 每次动作前先清掉已过期的固定目标，避免把动作推进早就没人用的旧窗口。
  await clearStalePinnedTarget();
  ...
  proxyServer = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === "POST" && url === "/pin") {
        ...
        return sendJson(res, ok ? 200 : 500, ok
          ? { ok: true, mode: "pinned", pinned, pinnedExpiresInMs: PINNED_TARGET_TTL_MS }
...
// ─────────────────────────────
//  PID 文件握手：认出「还在跑但插件丢了句柄」的遗留风铃进程
// ─────────────────────────────
export function readPidFile() {
  try {
    const raw = fs.readFileSync(PID_FILE, "ascii").toString().trim();
...
export function pidAlive(pid) {
  ...
  try {
    process.kill(pid, 0); // 信号 0 只探测存在性，不发送
    return true;
  } catch (e) {
    // EPERM = 进程存在但没权限发信号，也算活着；ESRCH = 不存在
...
async function waitStrayExit(pid, timeoutMs = 5000) {
...
function strayFenglingPid() {
  // 仅当插件没有自己的句柄时，才去认遗留球（自己 spawn 的球一律走 appProcess）
  if (appProcess) return null;
...
export function startFengling(bus, { allowDuringRestore = false } = {}) {
  ...
  // 发现遗留球（重启/重载后句柄丢了但球还活着）：先收掉它，避免桌面上出现两个风铃。
  const strayPid = strayFenglingPid();
  if (strayPid) {
    try {
      console.log("[闲不住] 发现遗留风铃进程 pid=" + strayPid + "，先收掉再拉新球");
      process.kill(strayPid);
    } catch (e) {
      console.warn("[闲不住] 清理遗留风铃失败:", e?.message || e);
    }
  }
...
export async function stopFengling() {
  ...
  if (!child) {
    // 插件没句柄，但桌面上可能还躺着遗留球：按 PID 文件把它收掉。
    const strayPid = strayFenglingPid();
    if (strayPid) {
      console.log("[闲不住] 收起遗留风铃 pid=" + strayPid);
      try {
        process.kill(strayPid);
      } catch (e) {
        console.warn("[闲不住] 收起遗留风铃失败:", e?.message || e);
        return { ok: false, error: e?.message || "停止遗留风铃失败" };
      }
      const exited = await waitStrayExit(strayPid);
      if (!exited) {
        // 没杀掉：保留 PID 文件，让状态仍能认出这个球，不误报已关。
        return { ok: false, error: "遗留风铃进程退出超时" };
      }
      clearPidFile();
      return { ok: true, message: "已停止", exited: true, stray: true };
    }
...
export function getFenglingState() {
  ...
  return {
    ok: true,
    running: !!appProcess || fusionActive || !!strayPid,
    stray: !!strayPid,
    pid: appProcess?.pid ? appProcess.pid : strayPid || null,
    fusionActive,
