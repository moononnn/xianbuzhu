...
export function publicHeart(heart) {
  ...
  return {
    ...
    status: heart.status,
    responded: Boolean(heart.respondedAt || heart.responseVisitId),
  };
}
...
export function getHeartSummary(data, now = Date.now()) {
  ...
  return {
    ...
    hearts: active.map((heart) => publicHeart(heart, data.heartSettings)),
    omittedCount: 0,
...
export async function generateAndSaveHeart({ entry, partnerId }) {
  ...
  return withDataLock(() => {
    ...
    return { ok: true, heart: publicHeart(heart, fresh.heartSettings) };
  });
}
