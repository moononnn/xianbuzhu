...
export function buildHeartPrompt({
  ...
  const seedsLine = (Array.isArray(interactionSeeds) && interactionSeeds.length)
    ? `
最近你们聊过的话题（可选作引子，让心意有来处；不必全用，挑一个自然的即可）：
${interactionSeeds.map((seed, i) => `${i + 1}. ${seed}`).join("\n")}`
    : "";
  const stylePair = `
同一件事，两种写法对照（你要的是第二种）：
- 别扭版（禁止模仿）：花插进窗边那个玻璃瓶里了，歪的，我懒得调，你回来看不顺眼再自己摆。
- 自然版（参考这版的落点和语气）：花我插窗边那个瓶子里了哦，摆得有点歪，回头看不顺眼再弄正，不急齁。`;
...
// 返回去重后的短引子列表（最多 3 条，每条截断到 ~60 字），供生成 prompt 注入。
          ...
          if (line) {
            try {
              ...
              if (text && !isHeartSeedInjected(text)) {
                const clean = text.replace(/\[[^\]]+\]/g, "").trim().slice(0, maxChars);
...
export async function generateAndSaveHeart({ entry, partnerId }) {
  ...
  const seeds = readRecentInteractionSeeds(partnerId);
