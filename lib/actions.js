...
// 文件预算豁免：动作全集聚合层（页面与悬浮球共用同一业务逻辑），拆分会造成两入口逻辑分叉
...
export async function performVisit(input, deps = {}) {
  ...
  return withDataLock(async () => {
    ...
    if (type === "prank" && itemId === "unplug") {
      ...
      // ── 互动 / 礼物：存为 completed，并进入普通投递队列 ──
      // 记录和扣款立即生效；消息本身等目标会话当前回复结束后再投递。
      ...
      visit.deliveryStatus = "queued";
      const delivery = createVisitDelivery({
        visit,
        text: pushText,
        sessionPath: targetSessionPath,
        userName,
      });
      if (!appendVisitDelivery(data, delivery)) {
        return {
          status: 500,
          body: { success: false, error: "投递队列写入失败，请重试" },
        };
      }
      if (returnHearts.length) {
      ...
      // 队列管理器使用宿主的 session_busy 作为最后一道门；事件与定时器只负责尽快重试。
      notifyDeliveryQueue(targetSessionPath);
    ...
    return {
      ...
      body: {
        ...
        deliveryStatus: visit.deliveryStatus || null,
        jar: data.jar,
