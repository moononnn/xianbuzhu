...
export function loadData() {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      // 兼容旧数据：清理 partnerConfig 中的非法键（历史版本校验宽松时沉淀的脏键）。
      // 若不清理，已保存配置会绕过 scanPartners 过滤，直接用于路径拼接
      if (data.partnerConfig && typeof data.partnerConfig === "object") {
        for (const id of Object.keys(data.partnerConfig)) {
          if (!isValidAgentId(id)) {
            delete data.partnerConfig[id];
            console.warn(`[闲不住] 清理非法伙伴 ID: ${JSON.stringify(id)}`);
          }
        }
      }
      // 确保 decorationItems 包含所有默认项（兼容旧数据缺少新项）
      const defaultDeco = defaultData().decorationItems || [];
      const existingIds = (data.decorationItems || []).map(function (i) {
        return i.id;
      });
...
export function saveData(data) {
  try {
    ...
    // 备份旧版本（.bak = 上一次成功写入的内容），再做原子替换
    // 顺序不能反：若先 rename 再 copy，.bak 会变成刚写入的当前内容，失去回退意义
    if (fs.existsSync(DATA_FILE)) {
      try {
        fs.copyFileSync(DATA_FILE, DATA_FILE + ".bak");
      } catch (e) {
        console.warn(
          "[闲不住] 备份 .bak 失败（本次写入继续，但回退能力可能不可用）:",
          e?.message || e,
        );
      }
    }
...
function performDailyReset(data) {
  ...
  for (const [partnerId, cfg] of Object.entries(data.partnerConfig || {})) {
    ...
    // 注意：75/38 是回归边界，不是展示档位（80/65/40/25），语义不同请勿对齐修改
...
// ─── 心情/精力档位（单一事实源） ───
// 阈值只在这里定义一次：describeMood/describeEnergy、sense-state、展板前端
// 全部通过 getMoodLevel/getEnergyLevel 取档位，杜绝多处手写漂移
// 档位数组按 min 从高到低排列，命中第一个满足条件的档位
export const MOOD_THRESHOLDS = [
  { min: 80, label: "心情很好", emoji: "😄" },
  { min: 65, label: "心情不错", emoji: "😌" },
  { min: 40, label: "心情平稳", emoji: "😐" },
  { min: 25, label: "有点闷", emoji: "😑" },
  { min: 0, label: "心情很差", emoji: "😩" },
];

export const ENERGY_THRESHOLDS = [
  { min: 70, label: "精力充沛", cls: "good" },
  { min: 40, label: "还行", cls: "mid" },
  { min: 20, label: "有点累", cls: "low" },
  { min: 0, label: "累坏了", cls: "low" },
];

export function getMoodLevel(mood) {
  if (!Number.isFinite(mood)) return MOOD_THRESHOLDS[MOOD_THRESHOLDS.length - 1];
  for (const t of MOOD_THRESHOLDS) {
    if (mood >= t.min) return t;
  }
  return MOOD_THRESHOLDS[MOOD_THRESHOLDS.length - 1];
}

export function getEnergyLevel(energy) {
  if (!Number.isFinite(energy)) return ENERGY_THRESHOLDS[ENERGY_THRESHOLDS.length - 1];
  for (const t of ENERGY_THRESHOLDS) {
    if (energy >= t.min) return t;
  }
  return ENERGY_THRESHOLDS[ENERGY_THRESHOLDS.length - 1];
}

// ─── 心情/精力模糊描述（注入用，不给硬数值，给 agent 留演绎空间） ───
export function describeMood(mood) {
  return getMoodLevel(mood).label;
}
...
// ─── 投递状态迁移（补投递队列用，纯函数可测） ───
// 推送失败的事件标记 deliveryPending 进入「信箱」，定时重试直到送达或放弃
// 字段约定：deliveryPending=true 待补投递；deliveryAttempts 已尝试次数；
//           deliveryDropped=true 尝试达上限放弃（记录保留，不再重试）

export function collectPendingDeliveries(data, limit = 20) {
  return (data.pendingVisits || [])
    .filter((v) => v.deliveryPending && !v.deliveryDropped)
    .slice(0, limit);
}

// 补投递成功：清除待投递标记，记录送达时间；返回是否找到该事件
export function markDelivered(data, visitId) {
  const v = (data.pendingVisits || []).find((x) => x.id === visitId);
  if (!v) return false;
  v.deliveryPending = false;
  v.deliveryDropped = false;
  v.deliveryDeliveredAt = nowISO();
  return true;
}

// 补投递失败：尝试次数 +1；达到上限标记放弃；返回是否已放弃
export function markDeliveryFailed(data, visitId, maxAttempts = 10) {
  const v = (data.pendingVisits || []).find((x) => x.id === visitId);
  if (!v) return false;
  v.deliveryAttempts = (v.deliveryAttempts || 0) + 1;
  if (v.deliveryAttempts >= maxAttempts) {
    v.deliveryDropped = true;
  }
  return v.deliveryDropped === true;
}
