...
export const DEFAULT_HEART_SETTINGS = {
  enabled: true, // 是否开启主动心意；频率只在开启后生效
...
function isActivePublicStatusId(statusId) {
  return DEFAULT_PUBLIC_STATUSES.some((item) => item.id === statusId);
}
...
export function getPublicStatusCollection(data, partnerId = "") {
  ensureStatusState(data);
  return (data.statusLibrary?.public || [])
    .filter((item) => isActivePublicStatusId(item.id))
    .map((item) => statusDto(item, isStatusUnlockedForPartner(data, partnerId, item)));
}
...
// 当前展示也必须经过同一套伙伴解锁判断；旧数据里残留的自动临时锁定状态不能继续冒充徽章。
function isCurrentStatusVisible(data, partnerId, status) {
  if (!status || status.scope !== "public") return true;
  // 只有当前仍在公共状态池里的定义才允许显示；旧库里已下架的公共 ID 不得被当成免费状态。
  if (!isActivePublicStatusId(status.id)) return false;
  const definition = getPublicStatusCollection(data, partnerId)
    .find((item) => item.id === status.id);
  return Boolean(definition && definition.unlocked !== false);
}
...
export function unlockPublicStatus(data, partnerId, statusId, now = Date.now()) {
  ...
  ensureStatusState(data);
  const status = (data.statusLibrary?.public || [])
    .find((item) => item.id === statusId && isActivePublicStatusId(item.id));
...
function normalizeCurrentStatus(current, now, data) {
  if (!current || typeof current !== "object") return null;
  // 已挂着的旧公共状态也跟随当前短文案，避免用户要等它自然过期才看见新版标签。
  const publicDefinition = DEFAULT_PUBLIC_STATUSES.find((item) => item.id === current.id)
    || (Array.isArray(data?.statusLibrary?.public)
      ? data.statusLibrary.public.find((item) => item?.id === current.id)
      : null);
...
function findCarryableStatus(data, partnerId, currentDate, now) {
  ...
  for (const date of dates) {
    ...
    return normalizeCurrentStatus(partnerDay.status, now, data);
  }
  return null;
}
...
export function getCurrentStatus(data, partnerId, now = Date.now()) {
  ...
  if (todayPartner?.status && typeof todayPartner.status === "object") {
    ...
    const current = normalizeCurrentStatus(todayPartner.status, now, data);
    return isCurrentStatusVisible(data, partnerId, current) ? current : null;
  }
  ...
  if (inherited && isCurrentStatusVisible(data, partnerId, inherited)) return inherited;
...
export function setPartnerStatus(data, partnerId, input = {}) {
  ...
  const source = input.source === "autonomous"
    ...
    : input.source === "partner"
      ...
      : "user";
  if (
    (source === "partner" || source === "autonomous")
    && data.statusSettings?.autonomousEnabled === false
  ) {
    return { ok: false, error: "伙伴自主状态功能已关闭，请先在设置里开启" };
  }
  ...
  if (requestedId) {
    ...
    // 不允许把付费公共状态改名伪装成临时专属状态，避免模型用 statusText 绕过解锁。
    const publicMatch = getPublicStatusCollection(data, partnerId)
      .find((item) => item.scope === "public" && item.text === text);
    if (publicMatch) {
      definition = publicMatch;
    } else {
  ...
  if (nextDefinition.scope === "public" && nextDefinition.unlocked === false) {
    return {
      ok: false,
...
export function isSelfInjectedPushText(text) {
  ...
  if (!t) return false;
  if (/^[📬📦🎁🧠✉]/.test(t)) {
    return /收到来自|给你带了东西|拍了拍你|趁你忙着时给你留了|的一份回礼|的一份礼物|的一条互动|的回应|一起回应/.test(t);
  }
  if (t === "重启！") return true;
...
// ─── 在所有伙伴中找用户最后操作过的会话窗口 ───
// 展板只需要近似跟随最近会话；短缓存避免每次轻量轮询都对所有目录做尾部扫描。
export const MOST_ACTIVE_AGENT_CACHE_TTL_MS = 10 * 1000;
let _mostActiveAgentCache = null;
let _mostActiveAgentCacheAt = 0;
let _mostActiveAgentCacheKey = "";

export function findMostActiveAgentId(agentIds, options = {}) {
  ...
  if (
    options?.force !== true
    && _mostActiveAgentCacheKey === cacheKey
    && now - _mostActiveAgentCacheAt < MOST_ACTIVE_AGENT_CACHE_TTL_MS
  ) {
    return _mostActiveAgentCache;
  }
  ...
  _mostActiveAgentCache = userBestId || fallbackId;
  _mostActiveAgentCacheAt = Date.now();
  _mostActiveAgentCacheKey = cacheKey;
  return _mostActiveAgentCache;
}

export function clearMostActiveAgentCache() {
  _mostActiveAgentCache = null;
  _mostActiveAgentCacheAt = 0;
  _mostActiveAgentCacheKey = "";
}
