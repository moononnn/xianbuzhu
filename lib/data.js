...
function performDailyReset(data) {
  ...
  for (const [partnerId, cfg] of Object.entries(data.partnerConfig || {})) {
    ...
    // 心情：睡前心情影响恢复量
    // mood > 60 → +10, mood 30~60 → +5, mood < 30 → +0
    const moodBefore = vars.mood || DEFAULT_VARIABLES.mood;
    let moodRecovery = 0;
    if (moodBefore > 60) moodRecovery = 10;
    else if (moodBefore >= 30) moodRecovery = 5;
    else moodRecovery = 0;
    vars.mood = Math.min(100, moodBefore + moodRecovery);
