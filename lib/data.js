...
// ─── 注入用上下文：心情（带原因）+ 精力 + 好感阶段 ───
// 示例：心情不错（昨天收到了一束花），精力还行，好感度 25（初识）
export function buildMoodContext(vars) {
  if (!vars) return '';
  let moodText = describeMood(vars.mood);
  if (vars.moodReason) moodText += `（${vars.moodReason}）`;
  const stage = getAffectionStage(vars.affection ?? 0);
  return `${moodText}，${describeEnergy(vars.energy)}，好感度 ${vars.affection ?? 0}（${stage.label}）`;
}
