...
export function loadData() {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      // 确保 decorationItems 按默认顺序重排（头像框在前、称号在后）。
      // 兼容旧数据缺项、乱序、重复；卡面是已移除的无效分类，连同旧数据一起清掉。
...
const LEGACY_CARD_DECORATION_IDS = new Set(["bg_warm", "bg_cool"]);

function isLegacyCardDecorationItem(item) {
  return Boolean(
    item
    && (item.type === "cardBg" || LEGACY_CARD_DECORATION_IDS.has(item.id)),
  );
}
...
// 统一收敛装饰数据。旧格式和已移除的 cardBg 都在这里处理，
// 只保留仍然有实际效果的头像框与称号，避免每个路由各写一套迁移。
export function normalizeDecorationState(value) {
  ...
  const titleOwned = uniqueStringList(owned.title);
  if (avatarFrame && !avatarOwned.includes(avatarFrame)) avatarOwned.push(avatarFrame);
  if (title && !titleOwned.includes(title)) titleOwned.push(title);

  const equippedAvatar = typeof equipped.avatarFrame === "string" ? equipped.avatarFrame.trim() : avatarFrame;
  const equippedTitle = typeof equipped.title === "string" ? equipped.title.trim() : title;
  if (equippedAvatar && !avatarOwned.includes(equippedAvatar)) avatarOwned.push(equippedAvatar);
  if (equippedTitle && !titleOwned.includes(equippedTitle)) titleOwned.push(equippedTitle);

  return {
    owned: { avatarFrame: avatarOwned, title: titleOwned },
    equipped: {
      avatarFrame: equippedAvatar && avatarOwned.includes(equippedAvatar) ? equippedAvatar : null,
      title: equippedTitle && titleOwned.includes(equippedTitle) ? equippedTitle : null,
    },
  };
}
