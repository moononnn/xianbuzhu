...
// ════════════════════════════════════════════
//  用户名清洗（限长 + 去控制字符，防脏输入；注意：这不能真正防止 prompt 注入）
// ════════════════════════════════════════════
...
export async function callLLM(prompt, options = {}) {
  ...
  // 组合取消信号：外部任务信号（事件超时/放弃时取消）+ 本调用超时兑底
  // 外部 signal 可真正中止 fetch，避免“旧任务不再写盘但网络请求还在跑”
...
// ════════════════════════════════════════════
export async function generateCrashReply(partnerId, signal) {
  const llmConfig = getLLMConfig();
  const desc = loadAgentDescription(partnerId);
  const data = loadData();
  const partnerName = data.partnerConfig?.[partnerId]?.name || partnerId;
...
export async function generateAndSaveNote(visit, partnerId, signal) {
  // 任务已超时失效：不再继续（显式检查，防止超时后仍写文件/写盘）
...
export function processVisitEvent(visit, partnerId) {
  const run = _visitQueue.then(() => {
    // 超时信号：到点后通知旧任务停止（Promise.race 只决定谁先结束，
    // 不会取消旧任务；不主动 abort 的话旧任务会在后台继续跑模型/写盘，
    // 与队列下一条任务并发，破坏串行一致性）
    const ctrl = new AbortController();
    const abortTimer = setTimeout(() => ctrl.abort(), VISIT_PROCESS_TIMEOUT);
    const task = processVisitEventInternal(visit, partnerId, ctrl.signal);
    // 两个定时器都保留引用，race 结束后一并清理，避免残留定时器存活 30s
    let timeoutTimer;
...
export async function processVisitEventInternal(visit, partnerId, signal) {
  // 已被超时放弃：不再处理
  ...
  try {
    ...
    if (partnerCfg?.variables) {
      ...
      if (additionalDeduction > 0) {
        partnerCfg.variables.energy -= additionalDeduction;
      ...
      // 写盘前检查：任务已超时失效时放弃写盘，避免与队列后续任务并发覆盖
      if (signal?.aborted) return;
      saveData(data0);
      console.log(
        `[闲不住] 变量更新: energy=${partnerCfg.variables.energy} mood=${partnerCfg.variables.mood} affection=${partnerCfg.variables.affection} (工作消耗: ${workConsumption}, 新增扣除: ${additionalDeduction})`,
      );
    }
    ...
    if (llmOk && shouldTriggerNote(visit)) {
      if (signal?.aborted) return;
...
export function shouldTriggerNote(visit) {
  ...
  if (visit.type === "gift") {
    ...
    if (price >= 130) return Math.random() < 0.8;
    if (price >= 80) return Math.random() < 0.45;
    if (price >= 70) return Math.random() < 0.4;
    if (price >= 30) return Math.random() < 0.2;
...
// ════════════════════════════════════════════
export async function generateBrainrot(signal) {
  const llmConfig = getLLMConfig();
