...
export function loadProviderConfigs() {
  try {
    if (!fs.existsSync(PROVIDERS_FILE)) {
      console.error('[闲不住] 未找到 added-models.yaml');
      return {};
    }
    ...
    for (const line of lines) {
      ...
      // 缩进6格且以 - 开头 → models 列表项
      if (indent === 6 && currentProvider && trimmed.startsWith('- ')) {
