...
import { loadData, saveData, nowISO, getAffectionStage, calcWorkConsumption, clampVariable, DEFAULT_VARIABLES, getToday } from './data.js';
import { getUserDisplayName, scanWorkStats, clearWorkStatsCache } from './activity.js';
...
export async function generateReply(visit, partnerId) {
  ...
  let stateDesc = '';
  if (vars) {
    const energyLabel = vars.energy >= 70 ? '精力充沛' : vars.energy >= 40 ? '还行' : vars.energy >= 20 ? '有点累' : '累坏了';
    const moodLabel = vars.mood >= 76 ? '心情很好' : vars.mood >= 51 ? '心情平稳' : vars.mood >= 26 ? '不太好' : '心情很差';
    const stage = getAffectionStage(vars.affection);
    stateDesc = `\n当前状态：精力 ${vars.energy}/100（${energyLabel}），心情 ${vars.mood}/100（${moodLabel}）\n好感度 ${vars.affection}，你们的关系阶段：${stage.label}`;
  }
...
// ════════════════════════════════════════════
export async function generateCrashReply(partnerId) {
  const llmConfig = getLLMConfig();
  const desc = loadAgentDescription(partnerId);
  const data = loadData();
  const partnerName = data.partnerConfig?.[partnerId]?.name || partnerId;
...
3. 最后**视情况**自然收尾：如果不知道刚才在写什么，就说「好了继续吧」这类简短结束语，**不要强行续接**
4. 回复的开头**必须**是崩溃效果，不能跳过直接说别的

直接输出完整内容，不要解释。不要在结尾暗示「开始写xxx」「继续写xxx」之类的内容。`;
...
const VARIABLE_RULES = {
  ...
  // 恶作剧（只扣精力和心情，不扣好感）
  unplug:  { energy: -15, mood: -12 },
  brainrot:{ energy: -8,  mood: -5 },
  // 礼物
...
function applyVariableChanges(vars, visit) {
  ...
  // 变量耦合：低谷放大器
  // 精力 < 30 时，心情增量打 5 折（只对正增量生效）
  if (energyBefore < 30 && rule.mood > 0) {
...
export async function processVisitEvent(visit, partnerId) {
  ...
  if (partnerCfg?.variables) {
    ...
    saveData(data0);
    console.log(`[闲不住] 变量更新: energy=${partnerCfg.variables.energy} mood=${partnerCfg.variables.mood} affection=${partnerCfg.variables.affection} (工作消耗: ${workConsumption}, 新增扣除: ${additionalDeduction})`);
  }

  // 1. 生成回应（如果已有 autoReply 则跳过，比如关机键的崩溃效果）
  const dataBefore = loadData();
  const existingVisit = dataBefore.pendingVisits?.find(v => v.id === visit.id);
