...
import { DEFAULT_VARIABLES, normalizeDecorationState } from './data.js';
...
export function mergeRefreshedPartners(oldConfig, scanned) {
  ...
  for (const [id, info] of Object.entries(scanned)) {
    ...
    if (oldCfg.decorations) merged.decorations = normalizeDecorationState(oldCfg.decorations);
    ...
    if (oldCfg.statusAutonomy) merged.statusAutonomy = oldCfg.statusAutonomy;
    if (Array.isArray(oldCfg.unlockedStatuses)) merged.unlockedStatuses = oldCfg.unlockedStatuses;
    if (Array.isArray(oldCfg.customStatuses)) merged.customStatuses = oldCfg.customStatuses;
