...
// 文件预算豁免：状态自主模块聚合状态读取、节流、提示词、模型回包和心跳协调，共享边界集中维护；后续职责再明显分叉时拆分。
...
// keep/unchanged 且没有新信号时，下一次只在下一生活日再问；真实活动/心情/事件变化仍可提前唤醒。
export const STATUS_AUTONOMY_KEEP_IDLE_INTERVAL_MS = 24 * 60 * 60 * 1000;
...
function buildStatusRules() {
  return [
    ...
    "只从 availableStatuses 里选最贴合的 statusId；未解锁的高级状态不会进入可选列表，也不能通过自动状态使用。只有现有状态都不合适时，才返回不超过8字的 statusText，而且必须是简短状态词，不写完整活动描述。不要编造资料里没有的事件、任务、心情或人物。空闲自选若想长期保持，可以使用 duration=until_changed。",
...
function activityForPartner(data, partnerId, activitySnapshot = {}) {
  ...
  // 事件是状态判断的重要触发源；带时间戳做指纹，避免 keep 后同一条旧事件每次都被当成新事件。
  activity.eventsFingerprint = JSON.stringify(
    (Array.isArray(today.events) ? today.events : []).slice(-12).map((event) => ({
      type: cleanActivityText(event?.type, 30),
      itemName: cleanActivityText(event?.itemName, 60),
      ts: cleanActivityText(event?.ts, 40),
    })),
  );
  return activity;
}

function statusSignalFingerprint(activity = {}, context = {}) {
  return JSON.stringify({
    activity: activity.fingerprint || "",
    events: activity.eventsFingerprint || "",
    moodBand: context.moodBand || "",
    energyBand: context.energyBand || "",
    conversationMeaningful: context.conversationMeaningful === true,
  });
}
...
function catalogForPrompt(catalog = {}) {
  const publicStatuses = Array.isArray(catalog.publicStatuses)
    ? catalog.publicStatuses.filter((item) => item && item.unlocked !== false)
    : [];
  const customStatuses = Array.isArray(catalog.customStatuses)
    ? catalog.customStatuses
    : [];
  return [
    ...publicStatuses,
    ...customStatuses,
...
function hasNewStatusSignal(state, context = {}) {
  if (typeof context.signalFingerprint === "string") {
    if (typeof state?.lastSignalFingerprint === "string") {
      return context.signalFingerprint !== state.lastSignalFingerprint;
    }
    // 老数据还没有信号指纹时，只让明确的活动/心情/精力/主对话信号打断旧节流；
    // 事件等旧字段可能会一直保留，不能在迁移第一轮后造成 90 分钟空转。
    return Boolean(
      context.activityChanged
      || context.moodChanged
      || context.energyChanged
      || context.conversationMeaningful,
    );
  }
  return Boolean(
    context.activityChanged
...
export function isAutonomousStatusDue(config, context, now = Date.now()) {
  ...
  // 无变化后的长节流只允许“新信号”提前打断；仍保留 90 分钟最短间隔。
  const signalChanged = hasNewStatusSignal(state, context);
  const canInterruptNoChange = (state.lastResult === "keep" || state.lastResult === "unchanged" || state.lastResult === "rejected") && signalChanged;
  const nextCheckAt = timestamp(state.nextCheckAt);
  if (Number.isFinite(nextCheckAt) && nextCheckAt > currentNow && !canInterruptNoChange) return false;
...
function chooseCandidates(data, now, activitySnapshot = {}) {
  ...
  for (let index = 0; index < ids.length; index += 1) {
    ...
    context.signalFingerprint = statusSignalFingerprint(activity, context);
...
function finishState(state, now, result, reason, error = "", activityFingerprint, context = {}) {
  state.checkingAt = null;
  state.claimId = null;
  state.lastCheckedAt = nowISO(now);
  // 业务规则拒绝（例如模型选了未解锁状态）不是模型通道故障：清掉失败计数，
  // 按无变化节奏等待下一次判断，避免错误选择触发指数退避或每几分钟空转。
  const noChangeResult = result === "keep" || result === "unchanged" || result === "rejected";
  const keepDelay = noChangeResult
    ? (Number(context.changesToday) === 0
      ? STATUS_AUTONOMY_KEEP_IDLE_INTERVAL_MS
      : STATUS_ROUTINE_INTERVAL_MS)
    : STATUS_AUTONOMY_INTERVAL_MS;
  state.nextCheckAt = nowISO(now + keepDelay);
  ...
  if (typeof context.signalFingerprint === "string") {
    state.lastSignalFingerprint = context.signalFingerprint;
  }
}
...
function isStatusDecisionRejection(error) {
  return /状态已经不在衣柜|还没解锁/.test(String(error || ""));
}
...
async function applyDecision(ctx, claim, decision, now) {
  return withDataLock(() => {
    ...
    if (state.claimId !== claim.claimId) return { ok: false, stale: true };
    if (data.statusSettings?.autonomousEnabled === false) {
      // 设置可能在模型请求期间被关闭；撤销 claim，但不把这次取消记成模型失败。
      state.checkingAt = null;
      state.claimId = null;
      ...
      const saved = saveData(data);
      return saved
        ? { ok: true, partnerId: claim.partnerId, skipped: "disabled" }
        : { ok: false, partnerId: claim.partnerId, error: "状态自检结果保存失败" };
    }
    if (!isVisiblePartner(data, claim.partnerId)) {
      finishState(state, now, "hidden", claim.context.reason, "", claim.activityFingerprint, claim.context);
      saveData(data);
    ...
    if (changedDuringCheck) {
      ...
      if (decision.statusId && !selectedStatus) {
      ...
      } else if (selectedStatus?.unlocked === false) {
        outcome = "rejected";
        ...
        if (!result.ok) {
          ...
          if (isStatusDecisionRejection(error)) decisionRejected = true;
          else failedDecision = true;
    ...
    if (decisionRejected) {
      finishState(state, now, "rejected", claim.context.reason, error, claim.activityFingerprint, claim.context);
    } else if (failedDecision) {
      finishFailure(state, now, error);
    } else {
      finishState(state, now, outcome, claim.context.reason, error, claim.activityFingerprint, claim.context);
    }
    const saved = saveData(data);
...
export async function runAutonomousStatusTick(ctx = {}, options = {}) {
  ...
  if (loadData().statusSettings?.autonomousEnabled === false) {
    return { ok: false, skipped: "disabled", checked: 0, results: [] };
  }
