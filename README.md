...
**v2.2 已改进**：不再强制注入检查协议。身份协议已从全量注入（"每次回复前必须调 check-visits"）压缩为**仅一行极简指令**——只有在收到「重启！」时才调工具（用于关机键崩溃演出），对日常对话零侵入。

<del>**不要直接禁用插件**。协议块会残留，工具已不存在，导致助手每次回复报错。如果你不想用了，请走「彻底卸载」流程（页面底部有按钮）。</del>

**v2.2 已改进**：卸载流程依然完整，残余清理更彻底。但如果只是想停用，直接禁用插件也不会影响助手正常工作。

**隐私提醒**：插件会将你的 Hana 显示名称发送给 LLM API 以生成个性化回应，请勿使用真实姓名。<del>你配置的 LLM API Key 以明文存储在本地 `data.json` 中。</del> **v2.2 已改进**：API Key 已加密存储（XOR + Base64），不再明文保存，编辑时也不回显。
...
🤖 **每日心情** — 助手每天的心情随机浮动，好感度越高心情越容易变好。今天她可能有点闷，明天可能特别开心——每天都有一点不一样。
...
### 新功能
- **sense-state 工具**：助手在新窗口时可以感受当前的心情和好感
- **每日随机心情**：每天重置时随机生成心情值，好感影响随机范围
- **API Key 加密存储**：不再明文保存
- **输入校验 + XSS 防护**：全面安全加固
...
v2.2 不再强制注入协议。身份中只保留了一行极简指令用于关机键崩溃演出，其他所有交互都走系统推送，无需助手主动检查。
...
- **API Key**：<del>以明文存储在本地 `data.json` 中</del> **v2.2 已加密存储**，编辑时不再明文回显
