...
- 默认自动跟随当前对话；手动选择时先选助手，再从她最近的 5 个活跃对话里挑一段
...
风铃是本插件**唯一**需要额外环境的功能，其他功能（展板、互动、送礼、恶作剧、光粒、小纸条）都不依赖它：

- **Python**：任意可用的 Python 3 即可，插件会自动探测（`C:\Python312`~`C:\Python314`、`python`、`python3`），不需要手动配置路径
- **PyQt6**：需要安装 PyQt6（含 QtSvg 模块）：

```bash
pip install PyQt6
```

- **验证过的系统**：Windows（主开发平台）和 Linux（CI 自动回归）
- **缺依赖会怎样**：只有风铃悬浮球不出现，其他功能一切正常。闲不住页面的风铃开关旁边会显示依赖检查结果，按提示装好再打开就行

**如果你的铃铛不响/不出来，先查这两样，多半是它们的问题，不是插件坏了。**
